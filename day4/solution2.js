...
//
  ...
  let heaviestSleeperHighestMinute;
  ...
  for(let i = 0; i < sortedData.length; i++){
    ...
    if(!guardsSchedule[currentActionId]) {
      guardsSchedule[currentActionId] = {longestSleep: 0, minuteOfOpportunity: null};
    }
    ...
    if(currentAction.string.includes("asleep")){
      ...
      for(let j = currentAction.minutes; j < nextAction.minutes; j++){
        ...
        if(bestMinutecount < guardsSchedule[currentActionId][j]){
          ...
          guardsSchedule[currentActionId].longestSleep = bestMinutecount;
          heaviestSleeperId = currentActionId;

        }
        // Find the longest moment for this guard to be asleep.
        // If that number is bigger than the current heaviest sleeper,
        // update it.
      }
    }
  }
      console
      console.log(heaviestSleeperId, guardsSchedule[heaviestSleeperId].minuteOfOpportunity)
